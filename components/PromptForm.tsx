...
const aspectRatioDisplayNames: Record<AspectRatio, string> = {
  ...
  [AspectRatio.TWO_THREE]: '2:3 (Portrait)',
  [AspectRatio.THREE_TWO]: '3:2 (Landscape)',
  [AspectRatio.THREE_FOUR]: '3:4 (Portrait)',
  [AspectRatio.FOUR_THREE]: '4:3 (Landscape)',
  [AspectRatio.ULTRAWIDE]: 'Ultrawide (21:9)',
};
...
const extractLastFrameBase64 = (file: File): Promise<string> => {
  return new Promise((resolve, reject) => {
    ...
    video.onerror = () => reject(new Error('Error loading video for frame extraction'));
  });
};
...
const VideoUpload: React.FC<{
  onSelect: (video: VideoFile, lastFrameBase64?: string) => void;
  onRemove?: () => void;
  video?: VideoFile | null;
  previewUrl?: string | null;
  label: React.ReactNode;
  sourceLabel?: string;
}> = ({onSelect, onRemove, video, previewUrl, label, sourceLabel}) => {
  const inputRef = useRef<HTMLInputElement>(null);
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      try {
        ...
        const lastFrame = await extractLastFrameBase64(file);
        onSelect(videoFile, lastFrame);
      } catch (error) {
        console.error('Error converting file or extracting frame:', error);
      } finally {
  ...
  if (video || previewUrl) {
    const finalUrl = video ? URL.createObjectURL(video.file) : previewUrl!;
    return (
      <div className="relative w-48 h-28 group">
        <video
          ...
          muted
          loop
          className="w-full h-full object-cover rounded-lg shadow-inner bg-black"
        />
        {sourceLabel && (
          <div className="absolute top-1 left-1 px-1.5 py-0.5 bg-indigo-600 text-[8px] font-black rounded uppercase tracking-tighter">
            {sourceLabel}
          </div>
        )}
        <button
  ...
  return (
    <button
      ...
      onClick={() => inputRef.current?.click()}
      className={`w-48 h-28 bg-gray-700/50 hover:bg-gray-700 border-2 border-dashed border-gray-600 rounded-lg flex flex-col items-center justify-center text-gray-400 hover:text-white transition-colors text-center ${isProcessing ? 'opacity-50 cursor-wait' : ''}`}>
      {isProcessing ? (
        <div className="w-6 h-6 border-2 border-white/20 border-t-white rounded-full animate-spin" />
      ) : (
        <PlusIcon className="w-6 h-6" />
      )}
      <span className="text-xs mt-1 px-2 leading-tight">{isProcessing ? 'Processing...' : label}</span>
      <input
        type="file"
        ref={inputRef}
        onChange={handleFileChange}
...
const PromptForm: React.FC<PromptFormProps> = ({
  ...
  const [inputVideoUrl, setInputVideoUrl] = useState<string | null>(
    initialValues?.inputVideoUrl ?? null,
  );
  const [lastFrameFromVideo, setLastFrameFromVideo] = useState<string | null>(
    initialValues?.lastFrameFromVideo ?? null,
  );
  ...
  useEffect(() => {
    if (initialValues) {
      ...
      setInputVideoUrl(initialValues.inputVideoUrl ?? null);
      setLastFrameFromVideo(initialValues.lastFrameFromVideo ?? null);
  ...
  const renderMediaUploads = () => {
        ...
        <div className="mb-3 p-4 bg-[#2c2c2e] rounded-xl border border-gray-700 flex flex-col items-center gap-5">
          <div className="w-full">
            ...
            <div className="flex flex-wrap items-center justify-center gap-3">
              ...
              {(referenceImages.length + (styleImage ? 1 : 0)) < 3 && (
                <ImageUpload
                  label="Add Asset"
    ...
    if (generationMode === GenerationMode.EXTEND_VIDEO) {
      const isExtendingResult = !!inputVideoObject;
      const isExtendingUpload = !!inputVideo;

      return (
        <div className="mb-3 p-4 bg-[#2c2c2e] rounded-xl border border-gray-700 flex flex-col items-center justify-center gap-3">
          <VideoUpload
            label={
              <>
                Add Video to Extend
            ...
            video={inputVideo}
            previewUrl={inputVideoUrl}
            sourceLabel={isExtendingResult ? "AI Result" : (isExtendingUpload ? "Manual Upload" : undefined)}
            onSelect={(video, frame) => {
              setInputVideo(video);
              setInputVideoObject(null); // Overriding previous result
              setInputVideoUrl(null);
              if (frame) setLastFrameFromVideo(frame);
            }}
            onRemove={() => {
              setInputVideo(null);
              setInputVideoObject(null);
          ...
          <p className="text-[10px] text-gray-500 italic text-center leading-relaxed">
            {isExtendingResult 
              ? "Extending your previous AI creation. Veo will continue the scene naturally."
              : (isExtendingUpload 
                ? "Extending a local upload. AI will start from the last captured frame."
                : "Select a video source to begin extending.")}
          </p>
        </div>
      );
    }
  ...
  switch (generationMode) {
    ...
    case GenerationMode.EXTEND_VIDEO:
      isSubmitDisabled = !inputVideoObject && !inputVideo;
      if (isSubmitDisabled) {
        tooltipText = 'An extension source (AI result or local upload) is required.';
      }
      break;
  }
